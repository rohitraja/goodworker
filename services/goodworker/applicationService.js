...
const applicationModel = require("../../models/betterplace/applications");
...
let findNearByCabs = function(city_id, lat, long, funCallback){
    ...
    asyncLib.waterfall([
        function(callback){
            ...
            applicationModel.getCabInDistanceRange(mysql, city_id, config.distance_range, lat, long, function(err, response){
                if(err){
                    callback(err);
                }else{
